...
class DataCsvInterface:
    """
        ...
        CONTINUOUS_NAMES: the columns to handled as continuous. If None, the non-cat, non-dep cols become con
    """
    ABS_CSV_PATH = str(Path(__file__).parents[0])
    CSV_FILE_NAME = "foreveralone_shuf.csv"
    DEPENDENT_NAME = "attempt_suicide"
    CATEGORY_NAMES = ['gender', 'sexuallity', 'race', 'virgin', 'prostitution_legal', 'pay_for_sex', 'social_fear',
                      'depressed', 'what_help_from_others', 'employment', 'edu_level', 'improve_yourself_how','income',  'bodyweight', ]
    CONTINUOUS_NAMES = ['age', 'friends']
